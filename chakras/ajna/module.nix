...
let
  cfg = config.services.soma.ajna;
  dnaBlueprint = builtins.fromJSON (builtins.readFile ../../config/dna_blueprints/dal_dna_v0.2.0_prime13_ajna_ubuntu.json);
in
{
  options.services.soma.ajna = {
    enable = mkEnableOption "Ajna chakra service (Prime 13 - Third Eye Perception)";
    
    port = mkOption {
      type = types.port;
      default = dnaBlueprint.endpoints.port;
      description = "Port for Ajna health endpoint";
    };
    
    basePath = mkOption {
      type = types.path;
      default = dnaBlueprint.directory_structure.base_path;
      description = "Base directory for Ajna chakra data";
    };
  };
  ...
  config = mkIf cfg.enable {
    ...
    systemd.services.soma-ajna = {
      description = "SOMA Ajna Chakra - ${dnaBlueprint.identity.name}";
      ...
      serviceConfig = {
        ...
        ExecStart = "${pkgs.bash}/bin/bash -c 'echo \"Ajna (Prime 13) running on port ${toString cfg.port}\"; while true; do sleep ${toString dnaBlueprint.swarm_awareness.heartbeat_interval_sec}; done'";
      };
    };

    warnings = [
      "SOMA Ajna service is a stub. Full Python agent implementation pending."
    ];
  };
}
